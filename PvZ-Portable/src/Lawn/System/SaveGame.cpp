...
static void SyncBoardBasePortable(PortableSaveContext& theContext, Board* theBoard)
{
	if (theContext.mReading)
	{
		...
		while (aReader.mOk && aReader.mPos < aReader.mSize)
		{
			...
			case BOARD_FIELD_BOSS_SPAWNED: ApplyFieldWithSync(aFieldData, aFieldSize, [&](PortableSaveContext& c){ c.SyncBool(theBoard->mBossSpawned); }); break;
			default: break;
			}
		}
	}
	else
	{
		...
		AppendFieldWithSync(aBlob, BOARD_FIELD_BOSS_SPAWNED, [&](PortableSaveContext& c){ c.SyncBool(theBoard->mBossSpawned); });
